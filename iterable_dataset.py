...
class CachedStreamingDataLoader:
    def __init__(self, records:Iterable[Any], fn: Callable[[Any, int], List[Any]], batch_size:int, cache_dir:str, prefix:str="tmp", ix_func:Callable[[int, int, int, Any], bool]=None):
        ...
        self.dl = DataLoader(ssd, batch_size, num_workers=0)
...
def all_gather_cpu(data, prefix="tmp", cleanup=True, ranks=None):
    """
    Pickles data from all processes, writes to disk, then read all of them on selected processes
    Args:
        data: any picklable object
    Returns:
        list[data]: list of data gathered from each rank
